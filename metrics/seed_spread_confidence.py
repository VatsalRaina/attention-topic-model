...
def main():
    ...
    sns.kdeplot(np.extract(labels.astype(np.bool), mutual_information),
                np.extract(labels.astype(np.bool), mean_target_deviation), cmap="Blues", shade=True, shade_lowest=False)
    # Negative examples
    sns.kdeplot(np.extract(np.invert(labels.astype(np.bool)), mutual_information),
                np.extract(np.invert(labels.astype(np.bool)), mean_target_deviation), cmap="Reds", shade=True, shade_lowest=False)
    plt.legend(['On-Topic', 'Off-Topic'])
    plt.xlabel("Mutual Information)")
