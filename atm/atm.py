...
#                                      epoch=epoch)
#
#     def _construct_contrastive_loss(self, logits, targets, is_in_domain, in_domain_precision=5., smoothing_val=1e-4,
#                          is_training=False):
#         """
#         Constrastive loss as described in the Prior Network paper
#         :param logits: Logit outputs of the prior network
#         :param targets: Sparse one-hot encoding of shape [batch_size, 2] (each column for on-topic and off-topic
#         respectively
#         :param is_in_domain: each value in the vector is 1 if example is in domain, 0 if out of domain.
...
#         in_domain_target_alphas = tf.abs(targets - smoothing_val) * in_domain_precision
#         out_of_domain_target_alphas = tf.ones([self.batch_size, 2], dtype=tf.float32)
#
#         # The first column (alpha1) corresponds to P_relevant, and the 2nd column (alpha2) corresponds to P_off_topic.
#
#         # Flatten is_in_domain vector
#         is_in_domain = tf.squeeze(is_in_domain)
#
#         # Construct the contrastive loss using is_in_domain to mask the appriopriate values
#         contrastive_loss = is_in_domain * self.kl_divergence_between_dirchlets(model_alphas, in_domain_target_alphas) \
#                            + (1 - is_in_domain) * self.kl_divergence_between_dirchlets(model_alphas,
#                                                                                        out_of_domain_target_alphas)
#
#         cost = tf.reduce_mean(contrastive_loss)  # Take mean batch-wise (over the number of examples)
#
#         if self._debug_mode > 1:
...
#                               "spkr": tf.FixedLenFeature([], tf.string),
#                               "q_id": tf.FixedLenFeature([], tf.int64),
#                               "is_in_domain": tf.FixedLenFeature([1], tf.float32)},
#             sequence_features={'response': tf.FixedLenSequenceFeature([], dtype=tf.int64),
#                                'prompt': tf.FixedLenSequenceFeature([], dtype=tf.int64)})
#
#         return contexts['targets'], contexts['is_in_domain'], contexts['q_id'], features['response'], features['prompt']
#
#     def _map_func(self, dataset, num_threads, capacity, augment=None):
#         dataset = dataset.map(lambda targets, is_in_domain, q_id, resp, prompt: (targets,
#                                                                                   is_in_domain,
#                                                                                   tf.cast(q_id, dtype=tf.int32),
#                                                                                   tf.cast(resp, dtype=tf.int32),
#                                                                                   tf.cast(prompt, dtype=tf.int32)),
#                               num_parallel_calls=num_threads).prefetch(capacity)
#
#         return dataset.map(lambda targets, is_in_domain, q_id, resp, prompt: (
#             targets, is_in_domain, q_id, resp, tf.size(resp), prompt, tf.size(prompt)),
#                            num_parallel_calls=num_threads).prefetch(capacity)
#
#     def _batch_func(self, dataset, batch_size, num_buckets=10, bucket_width=10):
#         # Bucket by source sequence length (buckets for lengths 0-9, 10-19, ...)
...
#                 padded_shapes=(
#                     tf.TensorShape([]),  # targets -- unused
#                     tf.TensorShape([1]),  # is in domain vector -- unused
#                     tf.TensorShape([]),  # q_id -- unused
#                     tf.TensorShape([None]),  # resp
#                     tf.TensorShape([]),  # resp len -- unused
#                     tf.TensorShape([None]),  # prompt
...
#                 padding_values=(
#                     0.0,  # targets -- unused
#                     0.0,  # is in domain vector -- unused
#                     np.int32(0),  # q_id -- unused
#                     np.int32(0),  # resp
#                     np.int32(0),  # resp len -- unused
#                     np.int32(0),  # resp len -- unused
...
#                 trn_cost, total_loss = self._construct_contrastive_loss(logits=trn_logits, targets=targets,
...
class ATMPriorNetwork(AttentionTopicModel):
    ...
    def _construct_contrastive_loss(self, logits_in_domain, logits_out_domain, targets_in_domain,
        ...
        cost = tf.reduce_mean(contr_loss_in_domain + contr_loss_out_domain) / 2.  # Take mean batch-wise (over the number of examples)
