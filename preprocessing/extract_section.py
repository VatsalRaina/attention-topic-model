...
def main(argv=None):
    ...
    for topic_sec_id, i in zip(topic_sec_ids, range(len(topic_sec_ids))):
    ...
    for key_sec_id, i in zip(key_sec_ids, range(len(key_sec_ids))):
    ...
    for i, x in zip(sorted_ids_tuples, range(len(sorted_ids_tuples))):
        for j, y in zip(sorted_key_tuples, range(len(sorted_key_tuples))):
    ...
    #  plt.ylabel('Entropy')
    # plt.xlim(0,379)
    ax1.plot([i / 379.0 for i in range(379)], sorted(entropy, reverse=True), c='r', lw=3.0)
    ax2.set_ylabel('AUC')
    #  ax2.set_xlim(0,76)
    #  ax1.set_xlim(0,379)
    ax2.plot([i / 76.0 for i in range(76)], aucs, c='g', ls='--', lw=3.0)
    ...
    for key, i in zip(key_sec_ids, range(len(key_sec_ids))):
        for topic, j in zip(topic_sec_ids, range(len(topic_sec_ids))):
