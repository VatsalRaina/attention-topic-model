...
"""
Create a k-fold split of the data for training on
"""
...
class ExamplesSet:
    ...
    def save_to(self, destination_dir):
        ...
        save_set(destination_dir, responses=self.responses, confidences=self.confidences, prompts=self.prompts,
                 grades=self.grades, speakers=self.speakers)
        return
...
def evenly_divide_distribution(distrib_dict, k):
    ...
    key_divisions = [[]] * k  # list of k empty lists
...
def main(args):
    ...
    responses_divided, confs_divided, prompts_divided, grades_divided, speakers_divided = [[]] * args.k, [[]] * args.k, \
                                                                                          [[]] * args.k, [[]] * args.k, \
                                                                                          [[]] * args.k
    ...
    example_set_divs = map(lambda x: ExamplesSet(*x),
    ...
    for hold_out_idx in range(args.k):
        ...
        held_out_div.save_to(os.path.join(args.destination_dir, 'dataset_' + str(hold_out_idx + 1), 'main'))
