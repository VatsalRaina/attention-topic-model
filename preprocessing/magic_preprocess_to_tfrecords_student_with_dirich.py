...
parser.add_argument('--dirich_params_file', type=str, default='alphas.txt')
...
def write_to_tfrecords(filename, destination_dir, responses, prompts, q_ids, grades, speakers, targets, predictions,
                       dirich_params, debug=False):
    # Check that all the input lists are of equal lengths
    assert len({len(responses), len(prompts), len(q_ids), len(grades), len(speakers), len(targets), len(predictions),
                len(dirich_params)}) == 1
    ...
    for response, prompt, q_id, grd, spkr, tgt, example_pred, example_dirich_params, idx in zip(responses, prompts,
        ...
        example = tf.train.SequenceExample(
            context=tf.train.Features(feature={
                ...
                'dirich_params': tfrecord_utils.float_feature(list(example_dirich_params)),
...
def main(args):
    ...
    if args.preprocessing_type == 'train':
        ...
        write_to_tfrecords('relevance.train.tfrecords', args.destination_dir, trn_responses, trn_prompts, trn_q_ids,
                           trn_grades, trn_speakers, trn_targets, trn_predictions, trn_alphas, debug=args.debug)
        ...
        write_to_tfrecords('relevance.test.tfrecords', args.destination_dir, responses, prompts, q_ids, grades,
                           speakers, targets, predictions, alphas, debug=args.debug)
